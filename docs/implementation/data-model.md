...
|---|---|---|
| `id` | INTEGER PK | |
| `name` | TEXT UNIQUE NOT NULL | Used in commands and autocomplete |
| `description` | TEXT NULL | |
| `gear_slots` | REAL NOT NULL DEFAULT 0 | Shadow Dark uses fractional slots in some homebrews. Default 0 means weightless. |
| `value_cp` | INTEGER NULL | Total value in copper pieces (1 gp = 100 cp). NULL = no value tracked. |
| `is_magical` | BOOLEAN NOT NULL | Determines which inventory it can live in |
...
| `item_id` | INTEGER FK items | Item must be `is_magical=true` |
...
1. **Magical sorting**: `inventory_entries.item_id` must reference an `items.is_magical = false`. `treasury_entries.item_id` must reference `is_magical = true`. Attempting the wrong combination produces an ephemeral error.
...
7. **Inventory capacity**: on `/inventory add`, `SUM(entry.quantity × item.gear_slots) + (new_quantity × new_item.gear_slots) ≤ location.max_gear_slots`.
