...
| **M8 — Player characters** | `/character` group: one character per player with lean stats (class/level/max HP/AC, six abilities + derived modifier table, gold, talents, optional spellcasting modifier), a hybrid catalog/freeform carried inventory with `max(10, STR)` carry capacity, a **known-spells** list backed by a seeded Shadow Dark spell reference (all Tier 1–5 wizard/priest plus 48 alignment-gated wizard spells; class-gated adds via the sheet's Manage Spells picker, no slash commands or custom spells), an ephemeral owner-only interactive sheet (edit modals + Manage Inventory + Manage Spells), and read-only `/character show`. Overrides the earlier "character sheets" non-goal. |
