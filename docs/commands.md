...
|---|---|---|
| `/items add name magical description? gear_slots? gp? sp? cp?` | Create a new catalog entry. `magical:true` marks it as treasury-eligible; `magical:false` marks it as inventory-eligible. Errors on duplicate name. | `/items add name:"Rope, 50ft" magical:false gear_slots:1 gp:1` |
| `/items info name` | Embed with description, gear slots, type, and value. | `/items info name:"Wand of Sparks"` |
| `/items edit name [fields…]` | Update any field. Providing any of `gp`/`sp`/`cp` replaces the whole value. Toggling `magical` is blocked if the item is currently referenced by inventory or treasury entries. | `/items edit name:"Rope, 50ft" gp:2` |
...
| `/items list magical?` | List the catalog, grouped by Common / Magical. Use `magical:true` or `magical:false` to filter to one group. | `/items list magical:true` |

**Tip:** `name` parameters autocomplete from the catalog as you type.
...
| `/inventory add location item quantity=1 notes?` | Add to a stack at a location. Item must exist in the catalog and be **non-magical**. Stacks combine automatically. Rejected if total slots would exceed capacity. | `/inventory add location:"Main Stash" item:"Rope, 50ft" quantity:3` |
...
```
/items add name:"Lantern" magical:false gear_slots:1 gp:5
...
| Command | What it does | Example |
|---|---|---|
| `/treasury add item tag?` | Register a magical item instance. The catalog entry must have `magical:true`. Use `tag` to distinguish duplicates ("chipped", etc.). Duplicates allowed. | `/treasury add item:"Wand of Sparks" tag:"chipped"` |
...
- Adding to inventory beyond a location's gear-slot capacity errors.
...
- Toggling a catalog item's `magical:` flag is blocked while it's referenced by inventory or treasury entries.
