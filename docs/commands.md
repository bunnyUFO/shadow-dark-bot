...
Stats covered: name, **ancestry**, class, **alignment**, level, **max HP**, AC, the six ability scores (with an auto-computed modifier table — Shadow Dark's `floor((score−10)/2)` clamped to ±4), gold (entered gp/sp/cp, stored as copper), **known languages**, a free-text **Talents** section, an optional **spellcasting** modifier (governing stat INT or WIS plus a manual spell-check bonus for talent-granted bonuses), and a **known spells** list.
...
**Manage Spells** opens the shared `/spells` browser in a character-management context. Inspect any spell and its detail view gains a **Learn** button (or **Forget**, if you already know it) alongside a **← Back to character** button. **Learn only succeeds for spells of your character's class** — attempting a wrong-class spell fails with a message (no alignment gating). Your class comes from your spellcasting stat (INT → wizard, WIS → priest), so set that via **Edit Talents & Casting** first. Known spells still appear (grouped by tier) on the sheet's Spells field. Spells-known limits are **not** enforced — the list is a tracker, not a rules engine.
