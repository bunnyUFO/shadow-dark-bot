...
- **`/character`** — your own player character (one per player, not guild-shared). `sheet` opens an ephemeral, owner-only interactive sheet split into three tabs — **Combat** (home: HP/AC, ability scores + modifiers, spellcasting, proficiencies, spells), **Inventory** (gold plus two location-backed stores — **Held** items with `max(10, STR)` capacity and a 10-slot **Guild Stash**), and **Roleplaying** (background, languages, talents, additional info). Each character's Held and Stash are real (private) inventory locations, created on character creation and kept in sync as you rename or change STR. Known spells come from a built-in Shadow Dark spell reference (all Tier 1–5 wizard/priest plus 48 alignment-gated wizard spells). **Manage Spells** is a dedicated, class-limited manager (a tier selector → your class's spells → a Learn button on the spell detail, plus Forget for known spells), separate from the read-only `/spells browse`. Any tier is learnable; wrong-class spells are rejected. The spellcasting stat and talent bonus are set together on **Edit Stats** (e.g. `WIS +1`). Identity (name, class/title, ancestry, alignment, background) is edited on **Edit Identity**; languages, talents, and free-form **Additional Info** live on **Edit Skills & Knowledge**. Freeform carried items can carry an optional description. From an item's detail you can **Give** it to another character's held items or to a storage location (a guild inventory location or your own stash) — capacity-checked, with magical and freeform items barred from guild inventory. `carry` adds items, and `show <member>` views anyone else's sheet read-only — held items only (the stash stays private), with a dropdown on the Combat tab to inspect any known spell and one on the Inventory tab to inspect any held item, in place.
